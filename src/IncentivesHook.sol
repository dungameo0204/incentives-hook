...
contract IncentivesHook is BaseHook, ERC20, BrevisApp {
    ...
    function _afterRemoveLiquidity(
        ...
        PoolId id = key.toId();
        bytes32 positionId = Position.calculatePositionKey(sender, params.tickLower, params.tickUpper, params.salt);
        uint128 liquidity = StateLibrary.getPositionLiquidity(poolManager, id, positionId);

        stakedLiquidity[id] -= liquidity;
        _updateReward(key, params.tickLower, params.tickUpper, sender, params.salt);
    ...
    function _updateReward(PoolKey calldata key, int24 tickLower, int24 tickUpper, address owner, bytes32 salt)
        internal
    {
        PoolId id = key.toId();
        _updateRewardsGrowthGlobal(key);
        ...
        uint128 liquidity = StateLibrary.getPositionLiquidity(poolManager, id, positionId);
        uint256 avaiable = (
            (rewardPerTokenInsideX128 - rewardPerTokenInsideInitialX128[id][positionId]) * uint256(liquidity)
        ) / FixedPoint128.Q128;
        ...
        returns (uint256 claimable)
    {
        ...
        uint256 rewardPerTokenInsideX128 = getRewardGrowthInside(tickLower, tickUpper, activeTick[key.toId()], key);

        console.log("rewardPerTokenInsideX128:", rewardPerTokenInsideX128);

        uint256 pending = ((rewardPerTokenInsideX128 - rewardPerTokenInsideInitialX128[id][positionId]) * liquidity)
            / FixedPoint128.Q128;
